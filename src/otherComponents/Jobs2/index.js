...
const Loading = () => (
  <div className="loader-container span-2" data-testid="loader">
    ...
    <li className="job-card">
      <Link to={`/jobs/${id}`} className="job-link">
        ...
        <div className="card-liner">
          ...
          <BsBriefcaseFill />
          <p>{employment_type}</p>
          <p className="ppa">{package_per_annum}</p>
        </div>
        <hr />
        <h1>Description</h1>
        <p>{job_description}</p>
...
const Failed = props => (
  <div className="span-2 failed">
    ...
    <button type="button" className="retry-btn" onClick={props.getData}>
      Retry
    </button>
  </div>
)
      ...
      <ListContext.Consumer>
        {value => {
          ...
          const Filters = props => {
            ...
            const func = () => {
              const myEl = document.getElementsByName('Salary Range')
              if (salary.length > 0) {
                myEl.forEach(item => {
                  item.checked = false
                })
